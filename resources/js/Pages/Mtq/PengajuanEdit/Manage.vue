<template>
    <AppLayout
        title="Manajemen Pengajuan Edit"
        :breadcrumb="[{ label: 'MTQ' }, { label: 'Manajemen Pengajuan Edit' }]"
    >
        <AppFlash />
        ...
        <div class="card">
            <div class="card-body p-0">
                <AppTable
                    ...
                    empty-text="Belum ada pengajuan edit"
                >
                    ...
                    <template #cell-aksi="{ row }">
                        <div class="d-flex align-items-center gap-1">
                            <template v-if="(row as PengajuanRow).status === 'menunggu'">
                                <template v-if="(row as PengajuanRow).peserta_status !== 'diverifikasi' || isSuperadmin">
                                    <button
                                        type="button"
                                ...
                                <span
                                    v-else
                                    class="badge text-bg-secondary small"
                                    title="Hanya superadmin yang dapat memproses pengajuan peserta terverifikasi"
                                >
                                    Perlu Superadmin
                                </span>
                            </template>
        ...
        <AppFormModal
            id="approveModal"
            size="sm"
            title="Setujui Pengajuan Edit"
            :processing="approveForm.processing"
            submit-label="Setujui"
            @submit="doApprove"
        >
            <p class="mb-3">
                Setujui pengajuan edit untuk peserta
                <strong>{{ approveTarget?.peserta_nama }}</strong>?
                Peserta akan dikembalikan ke status <strong>draft</strong>.
            </p>
            <AppTextarea
        ...
        <AppFormModal
            id="rejectModal"
            size="sm"
            title="Tolak Pengajuan Edit"
            :processing="rejectForm.processing"
            submit-label="Tolak Pengajuan"
            @submit="doReject"
        >
            <p class="mb-3">
                Tolak pengajuan edit untuk peserta
...
const { show, hide } = useBootstrapModal(['approveModal', 'rejectModal']);
...
const statusConfig: Record<string, { label: string; badgeClass: string; color: string }> = {
    menunggu:  { label: 'Menunggu',   badgeClass: 'text-bg-warning',   color: 'warning' },
    disetujui: { label: 'Disetujui',  badgeClass: 'text-bg-success',   color: 'success' },
    ditolak:   { label: 'Ditolak',    badgeClass: 'text-bg-danger',    color: 'danger'  },
};
...
function doApprove(): void {
    ...
    approveForm.post(`/mtq/pengajuan-edit/${approveTarget.value.id}/approve`, {
        preserveScroll: true,
        onSuccess: () => hide('approveModal'),
    });
}
...
function doReject(): void {
    ...
    rejectForm.post(`/mtq/pengajuan-edit/${rejectTarget.value.id}/reject`, {
        preserveScroll: true,
        onSuccess: () => hide('rejectModal'),
    });
}
</script>
